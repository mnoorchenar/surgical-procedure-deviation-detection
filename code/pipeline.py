...
# Set to full embedding dimension so no PCA compression occurs for BERT models.
# tinybert=256-d, sentencebert=384-d; using 384 means both keep all their dims
# (min(384, actual_dim) caps automatically inside _s03_encode_bert_pca).
# Classical encodings are disabled below, so this value only affects BERT PCA.
FEATURES_PER_COL = [384]

# -- Optuna general ------------------------------------------------------------
N_TRIALS                = 20     # 20 TPE trials is sufficient for all models; key models (ridge, xgboost) already ran with 50
...
OPTUNA_RF_N_EST_HIGH             = 250   # capped to reduce per-trial cost on large BERT feature matrices
...
# -- Optuna search bounds: ElasticNet -----------------------------------------
OPTUNA_EN_ALPHA_LOW     = 1e-3
OPTUNA_EN_ALPHA_HIGH    = 100.0
...
# Classical text encodings disabled: comparing BERT models only.
# Restore to ['label', 'tfidf', 'count'] to re-enable them.
CLASSICAL_ENCODINGS_WITH_N = []
BERT_ENCODINGS             = ['clinicalbert', 'sentencebert', 'tinybert']
ALL_MODELS                 = ['linear', 'ridge', 'lasso', 'elasticnet', 'randomforest', 'xgboost', 'lightgbm', 'mlp']
...
# tinybert cache is generated by distill_bert.py (run it once before Stage 02/03)
S02_TASKS = {
    1: ('clinicalbert', f'clinicalbert_{TEXT_COL}.npy'),
    2: ('sentencebert', f'sentencebert_{TEXT_COL}.npy'),
    3: ('tinybert',     f'tinybert_{TEXT_COL}.npy'),
}
...
def run_stage01():
    ...
    try:
        ...
        print(f"\n  [OK] Stage 01 complete.  Log -> {LOG_DIR}/01_preprocessing.log")
...
def _s02_compute_tinybert(texts):
    ...
    # Re-use TinySurgicalBERT class definition inline to avoid circular import
...
def run_stage02():
    ...
    print()
    for tid, (method, fname) in S02_TASKS.items():
        status = "[OK] exists" if _s02_task_is_done(tid) else "[X] missing"
...
def run_stage03():
    ...
    try:
        ...
        print(f"  [OK] Stage 03 complete.  Log -> {LOG_DIR}/03_fold_encoding.log")
...
def run_stage04():
    ...
    try:
        ...
        for fold in range(N_SPLITS):
            ...
            va_idx = fold_df[(fold_df['fold']==fold) & (fold_df['split']=='val')]['row_index'].values
            fold_indices[fold] = (tr_idx, va_idx)
        print(f"  [OK] Fold indices loaded ({N_SPLITS} folds)")
        ...
        if done_combos:
            ...
            if run_mode == 'r':
                ...
                print(f"  [OK] Resume mode -- {len(done_combos)} completed combo(s) will be skipped.")
                resume_mode = True
        else:
        ...
        def make_objective(name, Xtr, Xtr_sc, Xva, Xva_sc, ytr, yva):
            def obj(trial):
                if name in ('ridge', 'lasso'):
                    ...
                    p = {
                        'alpha':    trial.suggest_float('alpha',    OPTUNA_EN_ALPHA_LOW,    OPTUNA_EN_ALPHA_HIGH,    log=True),
                ...
                if name == 'mlp':
                    ...
                    # separate TF graph entries across 50 trials x 30 epochs).
                    train_ds = (tf.data.Dataset.from_tensor_slices((Xtr_f, ytr_f))
                                .shuffle(min(10_000, len(ytr_f)), seed=RANDOM_STATE)
                                .batch(MLP_BATCH_SIZE).prefetch(tf.data.AUTOTUNE))
                    val_ds   = (tf.data.Dataset.from_tensor_slices((Xva_f, yva_f))
            ...
            for (encoding, n) in all_combos:
                ...
                for model_name in MODEL_LIST:
                    ...
                    try:
                        ...
                        print(f"      [OK] MAE={metrics['mae']:>7.2f}  SMAPE={metrics['smape']:>6.2f}%  R={metrics['r2']:>6.4f}  RMSE={metrics['rmse']:>7.2f}  train={train_time_s:.2f}s  infer={infer_time_s:.4f}s")
